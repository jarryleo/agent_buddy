...
Coarse "where am I" lookup so the AI can answer weather / timezone / "near me" questions. Single action `get`. Always available — `BuiltinTool.location.isSupportedOnCurrentPlatform == true` on every platform.
...
- **Schema** lives in `ChatProvider._buildToolsSchema()` under `case 'location':`. The model's view: `action=get` (optional `timeout_ms`, default 10000). Returns `{action, location: {latitude, longitude, accuracyMeters?, city?, region?, country?, countryCode?, timezone?, isp?, source: gps|ip, fetchedAtMs}}`.
