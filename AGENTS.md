...
### Desktop single-instance lock (`SingleInstanceService`)
- The close-to-tray listener in `lib/main.dart::_setupDesktopWindow` keeps the OS process alive after ✕, so a second shortcut / autostart launch would spawn a brand-new engine that races the hidden primary for SharedPreferences / Google Sheets tokens / Hive boxes and frequently renders a blank window. `SingleInstanceService` (`lib/services/single_instance_service.dart`) is the gate that turns the second launch into a "raise the existing window" command.
- **Wire protocol** — a `dart:io` `ServerSocket` bound to `127.0.0.1:0` (OS-assigned ephemeral port, never shared across processes) plus a per-user lock file under `getApplicationSupportDirectory()` named `instance.lock`. The lock file is the *only* source of truth for the primary's port — avoiding cross-platform Dart / Winsock quirks (`SO_REUSEADDR` semantics differ between Linux / Windows, fixed-port shared-bind is not portable). The protocol itself is line-oriented — every command is `COMMAND\n`, current set is just `SHOW`. No new package dependency was introduced (`path_provider` was already pulled in for the `working/` directory bridge).
  - `acquire()` — reads `<lock>/instance.lock`. If the file is empty (cold start) OR the recorded port fails `Socket.connect` (stale lock from a crashed primary, deleted best-effort), it binds a fresh ephemeral `ServerSocket` and writes the new port to the file. Returns `true` on success / `false` if the probe succeeded (someone else is the primary).
  - `sendShowToExisting({timeout})` — second-instance API: read the lock file, `Socket.connect` to the recorded port, send `SHOW\n`, return whether delivery succeeded. Short `timeout` (default 800 ms, configurable via constructor) so a brief socket hiccup doesn't park the secondary forever. Re-reads the file once on probe failure in case a brand-new primary raced the connect to land between our read and our probe.
  - `setOnShowRequested(handler)` — called inside `windowManager.waitUntilReadyToShow`'s callback in `lib/main.dart::_setupDesktopWindow` once the plugin is fully initialised. Any commands buffered before the handler is wired (rare but possible when a second launch races this `main()`) are replayed in arrival order, so a SHOW that arrived during boot still fires immediately on first show.
- **Lock file write is atomic** — the service writes to `instance.lock.tmp` first, then `rename`s into place, so a racing read never observes a partially written port number.
- **`exit(0)` from inside `_setupDesktopWindow`** — the second process never touches Hive or the settings provider; the call returns a `Never` so the `await` chain in `mainApp()` is abandoned mid-flight. Same pattern as `tray_service.dart::_exitApp`. The lock file is intentionally *not* deleted on this path — the next legitimate launch detects "no listener" via the probe and reclaims the slot.
- **Mobile / web** — `acquire()` returns `true` without touching the filesystem (`!isDesktop()` early-out); `sendShowToExisting` returns `false`. The pet-window sub-engine inside the same OS process re-enters the lock safely via the idempotency check (`if (_server != null) return true`) — no second bind.
- **Tests** — `test/single_instance_service_test.dart` covers the protocol end-to-end: first/second acquire against the same lock file, idempotent re-acquire, stale-lock detection (file written to a port no one is listening on), dispose-releases-lock-file, `sendShowToExisting` returns false with no lock file / true once acquired, unknown-command tolerance, buffering + replay when the handler is wired late, post-ready immediate dispatch, and a thrown-handler does not break the loop. Each test allocates a fresh temp dir + lock file so parallel runs don't collide.

### Desktop auto-start (`AutostartService`)
- `lib/services/platform/autostart_service.dart` is the cross-platform abstraction. The general settings tab uses `isAutostartSupportedOnCurrentPlatform()` to gate the toggle (only desktop); mobile / web never see the row.
- Per-OS concrete impls live in `lib/services/platform/autostart_service_io.dart`; no new package dependencies — every impl talks to the OS-native startup hook directly via `dart:io`:
  - **Windows** — `WindowsAutostartService`. Writes a `REG_SZ` value under `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` via `reg.exe` (HKCU = current user, no elevation required). The value name defaults to `AgentBuddy`; the value is the absolute path of `Platform.resolvedExecutable`, quoted so paths with spaces survive the registry parse on next reboot. Sanitization strips `\`, `/`, `:`, `*`, `?`, `"`, `<`, `>`, `|`, and control characters from the value name.
  - **macOS** — `MacAutostartService`. Drops a hand-rolled LaunchAgent plist into `~/Library/LaunchAgents/<bundle-id>.plist` with `RunAtLoad=true` + `ProcessType=Interactive` (so the launched app is a normal UI app rather than a headless helper). XML-escapes the executable path before writing.
  - **Linux** — `LinuxAutostartService`. Writes an XDG-compliant `.desktop` entry into `~/.config/autostart/<name>.desktop` (honours `$XDG_CONFIG_HOME`). Minimal entry: `Type=Application`, `Name`, `Exec` (quoted when the path contains a space), `Terminal=false`, `X-GNOME-Autostart-enabled=true`. Per the XDG spec, disabling means *deleting the file* (not flipping `Hidden=true`), so the "Show applications" tool lists the entry exactly when it's registered.
- `lib/services/platform/autostart_service_stub.dart` is the no-op for mobile / web; `isSupported == false` so the settings row never renders.
- **Wiring** — `main.dart` constructs the singleton via `createAutostartService()` (a factory that picks the right impl at runtime) and passes it to `SettingsProvider(storage, googleSheets, autostart)..load()..attachAutostartService(autostart)`. The trailing `attachAutostartService` re-applies the persisted preference on every cold start (e.g. the user wiped the autostart file from another tool — we put it back) and is a no-op when the preference is `false`.
- **SettingsProvider.setAutoStartEnabled** forwards to the service. `null` = the OS write failed; the caller (the settings tab) shows a localized snackbar. `true` / `false` mirror the OS verdict, not the user input — protects against the OS rejecting our write (e.g. read-only `~/Library/LaunchAgents`).
- **Tests** — `test/autostart_service_test.dart` (factory + stub + macOS/Linux impls round-trip with `homeDirectoryOverride` + plist XML-escape + bundle-id sanitization + path-quoting) + `test/autostart_settings_provider_test.dart` (default false, persistence, no-service path, service-forwarding, failure rollback, boot-time re-apply via `attachAutostartService`, unsupported-service skip, backward-compat constructor signature).
...
### Voice input + read-aloud (`stts`)
- The `stts` plugin (`llfbandit/stts`, see `pubspec.yaml`) ships *both* STT (`Stt` class) and TTS (`Tts` class) on every Flutter target (`android.speech.SpeechRecognizer` / `TextToSpeech` on Android, `SFSpeechRecognizer` / `AVSpeechSynthesizer` on iOS / macOS, SAPI on Windows, browser's `SpeechRecognition` / `SpeechSynthesis` on web). We use both halves:
  - **STT** is the chat-input mic; see the rest of this section.
  - **TTS** drives the per-bubble "read aloud" button on assistant messages. Implementation in `lib/services/tts_service.dart`; rendered by `MessageBubble._buildTtsSpeaker` (overlay on the bottom-right of the bubble).
- Pinned at `^1.2.6` because 1.3.x requires Dart ≥ 3.12 / Flutter ≥ 3.44, which is newer than our `sdk: ^3.11.1` floor.
- **`stts.start()` is fire-and-forget** (returns `Future<void>` immediately), unlike `speech_to_text`'s `listen()` which returned `bool`. `VoiceServiceImpl` (`lib/services/platform/voice_service_impl.dart`) therefore uses a 1.5s **liveness probe** (`_livenessProbe` Completer) — it waits for the corresponding `SttState.start` event (Android: `onReadyForSpeech`, iOS: `audioEngine.start()` post `setActive(true)`, Windows: SAPI callback). Any `onError` that fires inside the grace window short-circuits as a real failure; a timeout with no callback surfaces as `VoiceError.unknown`.
- **State translation layer.** `stts` only emits two states (`SttState.start` / `SttState.stop`). The wrapper maps them to the `speech_to_text`-style strings the chat input already listens for — `start` → `'listening'`, `stop` → `'notListening'`. Final results carry `isFinal=true` so the UI doesn't need a separate `'done'` event.
- **Permission flow stays dual-source.** `stts.hasPermission()` is a bare boolean with no notion of "permanently denied". The wrapper still uses `permission_handler.microphone` for the `granted` / `denied` / `permanentlyDenied` distinction so the UI can offer the "open system settings" affordance. `permission_handler` is already in `pubspec.yaml` for the calendar / reminders / location flows so we keep it.
- **No engine amplitude.** `stts`'s Android side intentionally overrides `onRmsChanged` as a no-op, and iOS / macOS / Windows don't expose RMS at all. The chat input's existing synthetic-level bump (random pulse on every new partial + 60ms decay timer) is now the **only** level-meter source — previously it was just a fallback for the WinRT backend. The chat input already wires this in via `_bumpSyntheticLevel` / `_voiceLevelDecayTimer` so no UI changes were needed; only the doc-comments were updated.
- **Locale IDs are BCP-47** (`zh-CN`, `en-US`), not `zh_CN` / `en_US` like `speech_to_text` wanted. `_localeIdForSpeech` in `lib/widgets/chat_input.dart` maps the canonicalized app locale (`'zh'` / `'en'`) accordingly. Unknown app locales fall through to `null` so the recognizer picks its default — which on Windows is the system locale and is frequently wrong for our users, so we only fall through for app locales we don't know about, never for `'zh'` / `'en'`.
- **Liveness on partial results.** The chat input also flips `_voiceActuallyStarted` on the *first non-empty partial* (not just on `onStatus('listening')`). This is critical because on Windows the SAPI backend sometimes streams results before firing any state event — without the partial-driven path the long-press-release would bail and orphan the session.
- Unit-tested in `test/voice_service_impl_test.dart` via a `_FakeSttPlatform` (subclass of `SttPlatformInterface` with controllable `StreamController`-backed `onStateChanged` / `onResultChanged`); existing `test/voice_service_test.dart` + `test/chat_input_voice_test.dart` cover the UI contract.
...
### Read-aloud / TTS (`stts`)
- `lib/services/tts_service.dart` wraps `stts.Tts` (the same package as STT; same `^1.2.6` pin). Plain class with a constructor-injectable `Tts? engine` for tests, plus a `Provider<TtsService>` registered in `main.dart`. No `ChangeNotifier` — the bubble binds to two `ValueNotifier`s directly so unrelated subscribers don't rebuild.
- **State surface** — two `ValueNotifier`s: `speakingMessageId` (the `ChatMessage.id` currently being spoken, or `null` for idle) and `isPausedNotifier`. The bubble compares its own `widget.message.id` against the current speaking id to decide which of three icons to render (`volume_up` idle / `stop` speaking / `play_arrow` paused).
- **Engine state stream** — `SttState` / `TtsState` semantics differ: STT emits two states, TTS emits three (`stop` / `start` / `pause`). The wrapper translates `pause` into the `isPausedNotifier` flag and `stop` into clearing `speakingMessageId`.
- **Single active speaker** — `speak(messageId, text, {localeId})` toggles playback when tapped on the same bubble, or stops + restarts when tapped on a different bubble. Mirrors what users expect from a chat-app read-aloud: one paragraph at a time, no overlaps. Tapping `stop` while speaking is the explicit path; the `stop` event from the engine is the natural-finish path.
- **Locale** — uses the same BCP-47 mapping as the chat-input STT (`'zh'` → `'zh-CN'`, `'en'` → `'en-US'`); without an explicit `setLanguage(...)` call the SAPI default on Windows falls back to the system locale, which is frequently wrong for bilingual users.
- **Markdown stripping** — `MessageBubble._stripMarkdownForSpeech` removes inline code ticks, heading hashes, list bullets, and `**bold**` / `*italic*` markers before handing the text to the engine. Fenced code blocks are kept verbatim (users often ask the model to *write* code, and a TTS read of the code body is more useful than silence). Stripping uses `replaceAllMapped` for capture-group expansion — `replaceAll(..., String)` treats `$1` as a literal, which is the easy mistake to make here.
- **Unsupported-platform path** — `TtsService.initialize()` probes `stts.isSupported()`. The result is published via a `ValueNotifier<bool>` (`isSupportedNotifier`) so the bubble can rebuild the moment the verdict lands. Until the probe resolves, `isSupported` is `false` and the speaker is hidden.
- **Eager init at startup** — `main.dart` constructs the singleton `TtsService` *before* `runApp` and fires `..initialize()` on it, then registers it via `Provider<TtsService>.value`. Without this, the bubble's first render sees `isSupported == false` and hides the speaker — and the only event that would have called `initialize()` (the user tapping a button that doesn't exist) never fires, so the speaker stays button-less forever. This bit us hard on Windows where SAPI is bundled but the probe is async. The probe is fast on every supported platform (≤ 100 ms) so the startup cost is negligible.
- **Late-probe reentry** — the bubble's post-frame callback in `initState` seeds the cached `_ttsIsSupported` from the live notifier and calls `setState` if it differs from `false`. This covers the rare race where the first render sneaks in before the probe lands (e.g. a slow first-build platform); the notifier listener then keeps the snapshot in sync for any later flips.
- **Test seam** — the fake `TtsPlatformInterface` in `test/tts_service_test.dart` mirrors what the real platform does after each call: `stop()` fires `TtsState.stop`, `pause()` fires `TtsState.pause`, `resume()` fires `TtsState.start`. Without that mirror, idempotency tests (calling `pause()` twice) would see the wrapper's `isPausedNotifier` stuck on `false` and re-fire the engine.
- **Bubble widget integration** — the speaker is positioned at `right: 2, bottom: 2` inside the bubble's `Stack` overlay so it sits at the bubble's bottom-right without affecting the markdown content's layout. A 28×28 circular pill with a 50%-opaque bubble-color background gives a visual focus affordance without a [Tooltip] wrapper (which would install its own `LongPressGestureRecognizer` and fight other gesture recognizers).
...
### Mobile file tool (SAF + working directory)
...
On mobile the model is gated to two path schemes so it can never reach the raw filesystem. The model **defaults to operating on the user-selected working directory or on files the user explicitly picked** — there are no other sandbox roots.
...
- `working://<rel>` or a bare relative path — the user-selected working directory (the folder the user picked via the chat toolbar's `FilePicker.platform.getDirectoryPath()`). Mirrors desktop's relative-path behaviour: `foo/bar.txt` resolves to `<workingDir>/foo/bar.txt`; `working://` resolves to the root; `working://sub` to `<workingDir>/sub`. The model never sees the absolute OS path. Resolved by `FileServiceImpl._resolveWorkingPath`; `..` is rejected by the same `p.normalize` + `startsWith` check so the model can't escape. `ToolService.file` is wired to a lazy `workingDirectoryLookup: () => storage.modelWorkingDirectory` so the FileService always sees the latest value without manual sync. No working directory → bare-relative / `working://` paths throw a friendly `no working directory configured on mobile` error pointing the model at `pick`.
...
- `read_attr` / `read` / `write` / `append` work against both schemes (`picker://`, `working://` / bare relative).
- `delete` / `rename` / `list_dir` work against `working://` only. Picker paths are rejected with a friendly `release` hint because the OS picker grants per-URI access for read/write, not arbitrary delete on the user's filesystem.
...
Anthropic-compatible providers (MiniMax / Anthropic / any other Anthropic-protocol gateway) can opt into proactive prompt caching by toggling `ModelProvider.promptCacheEnabled`. When on, `ApiService` attaches `cache_control: {"type": "ephemeral"}` to static content so the server can reuse it across turns (cache TTL = 5 min, refresh-on-hit). Follows the MiniMax docs at https://platform.minimaxi.com/docs/api-reference/anthropic-api-compatible-cache.

- **Model layer** — `ModelProvider.promptCacheEnabled` (default `false`). Serialized to JSON only when `true` so v1 rows round-trip identically. The OpenAI-protocol transport ignores the flag (OpenAI doesn't honour `cache_control`); the local LLM transport ignores it too (llama.cpp has no cache layer).
- **Wire markers** (`lib/services/api_service.dart`) — single marker per category on the LAST block (per the docs "you only need a single cache breakpoint at the end of the static content and the system finds the longest matching prefix"). Cached by category:
  - `tools` — last tool definition (covers tools + everything above).
  - `system` — last text block. When the toggle is off, the legacy flat-string `system` shape is preserved byte-for-byte; when on, the wire switches to the array-of-blocks form (the only way to attach `cache_control`).
  - `messages` — last user-message block. Text-only user messages get converted from a flat string into a single-block content array; multi-part user messages (text + image/document) get the marker on the trailing block. Assistant turns are deliberately NOT marked — they change every round, so caching them would mostly invalidate the prefix anyway.
  - One full request cycle honours up to 4 breakpoints per the docs; we use exactly 3 (tools / system / last-user-message) so the model still has a slot left if it later wants to add another breakpoint.
- **Usage parsing** — Anthropic SSE surfaces `usage` on `message_start` (initial counts) and updates on `message_delta` (final `output_tokens`). `ApiService._runAnthropicTurn` collects both into local closure vars and yields an `OrchestratorEvent.usage(TurnUsage)` right before the `turnDone` sentinel. The orchestrator (`ToolOrchestrator.run`) re-emits a single `usage` event after the final round so the chat UI sees one snapshot per turn (the content-bearing one — earlier tool-call rounds' usage is discarded as less meaningful for the footer).
- **`TurnUsage`** (`lib/services/tool_orchestrator.dart`) — `{inputTokens, cacheCreationInputTokens, cacheReadInputTokens, outputTokens}`. Helper `cacheHit` = `cacheReadInputTokens > 0`; helper `totalInputTokens` matches the docs' "total = cache_read + cache_creation + input" formula.
- **`MessageMetrics`** (`lib/models/message.dart`) — gained `cacheUncachedInputTokens` / `cacheCreationInputTokens` / `cacheReadInputTokens` (all default 0; only serialized when the server reported usage, so pre-cache records keep round-tripping identically). `hasServerUsage` flag gates the bubble footer chip.
- **Stream surface** — `StreamEvent.usage(...)` factory + `usageInputTokens` / `usageCacheCreationInputTokens` / `usageCacheReadInputTokens` / `usageOutputTokens` fields. The single-turn chat path (`_streamAnthropicSingleTurn`) and the orchestrator path (`streamChat` switch on `OrchestratorEventKind.usage`) both forward it.
- **ChatProvider** — listens for `event.type == 'usage'`, updates `MessageMetrics` in place via `_replaceMessages`, and includes the latest cache counts in the final metrics writeback at the end of the stream so the persisted footer survives an app restart.
- **UI** — `_PromptCacheSwitch` in `lib/pages/add_provider_page.dart`, only rendered when `protocol == ProviderProtocol.anthropic`. Switches `promptCacheEnabled`, persists via the Add/Edit save path. Helper text explains the cost / latency tradeoff (cheaper read, faster TTFT after the first request; writes cost slightly more than regular input).
- **Bubble footer** (`lib/widgets/message_bubble.dart`) — `⚡ cache hit N token` chip after the Σ total. Gated on `metrics.hasServerUsage && metrics.cacheReadInputTokens > 0`, so the chip is silent on cold requests / transports that don't report usage.
- **l10n** — `providerPromptCache` + `providerPromptCacheHelper` (provider toggle) and `messageMetricCacheHit` (footer chip) in `app_en.arb` / `app_zh.arb` + the matching generated localizations files.
- **Tests** — `test/anthropic_prompt_cache_test.dart` (15 tests): `ModelProvider.promptCacheEnabled` JSON round-trip + legacy defaults + omitting-when-false, Anthropic `cache_control` wire format (text-only / multi-part user / assistant-no-marker / tools-only-last / system-only-last / flat-string-when-off / OpenAI-ignores-flag), Anthropic SSE usage parsing (message_start + message_delta), `MessageMetrics` cache fields persistence. Existing `chat_tools_api_test.dart`, `chat_attachment_path_test.dart`, `sub_agent_*_test.dart`, `api_service_orchestrator_error_test.dart`, `chat_retry_*` tests continue to pass.
