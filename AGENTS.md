...
### Voice input (`stts`)
- The `stts` plugin (`llfbandit/stts`, see `pubspec.yaml`) is the cross-platform STT wrapper around the OS-native recognizer (`android.speech.SpeechRecognizer` on Android, `SFSpeechRecognizer` on iOS / macOS, SAPI on Windows, browser's `SpeechRecognition` on web). It's a thin layer over each platform — no Dart-side recognizer, no on-device GGUF model. Pinned at `^1.2.6` because 1.3.x requires Dart ≥ 3.12 / Flutter ≥ 3.44, which is newer than our `sdk: ^3.11.1` floor.
