...
- `read_attr` / `read` / `write` / `append` work against all three schemes (`app://`, `picker://`, `working://` / bare relative).
- `delete` / `rename` / `list_dir` work against `app://` and `working://`. Picker paths are rejected with a friendly `release` hint because the OS picker grants per-URI access for read/write, not arbitrary delete on the user's filesystem.
