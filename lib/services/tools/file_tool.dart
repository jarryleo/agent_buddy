...
class FileTool extends ToolBase {
  ...
  static const String _desktopDescription =
      '管理电脑文件(读/写/删/改名/列目录/查属性)。仅 Windows / macOS / Linux 可用。改代码用 action=edit 按 1-based 行号替换，支持批量编辑;read 支持 offset_lines/max_lines 分页+pattern 当 grep。';
  ...
  String get description {
    if (isMobileForRuntime()) {
      return '管理设备文件。手机: 默认操作工作目录(相对路径或 working://),或用 action=pick 打开系统选择器读/写手机上的任意文件(无需 Android 权限)。电脑走原桌面端逻辑。改代码用 action=edit 按 1-based 行号替换，支持批量编辑;read 支持 offset_lines/max_lines 分页+pattern 当 grep。';
    }
...
- edit {path, edits:[{start_line, end_line?, content}]}: 按行号替换，编辑范围包含首尾行
...
edit 返回:
- 成功: {action:"edit", ok:true, applied:数量, diff:[{start_line,end_line,replacements}]}
- 失败: 工具错误中包含 {action:"edit", ok:false, error_code, failed_index, start_line?, end_line?}
...
约束:
- edit 的 start_line/end_line 为 1-based，end_line 省略时只编辑 start_line，end_line 不能小于 start_line
- edit 一次提交多个 edits 时按行号从大到小应用，任何一个失败都不会写盘
- content 为空删除指定行；要在某行插入内容而保留原行，content 必须同时包含原行文本
- 每个文件编辑完成后都会重新读取并校验写入结果
- 所有路径禁止 .. 跳出工作目录

最佳实践:
- **改代码必须用 action=edit**，先 read 获取 1-based 行号，再传 start_line/end_line/content；不要 read+write 整文件。
- 要替换多行，end_line 传最后一行；要删除行，content 传空字符串。
- 要插入新内容，使用 start_line=end_line，content 中保留原行文本并加入新内容。
...
''';
      ...
      'parameters': {
        ...
        'properties': {
          ...
          'edits': {
            'type': 'array',
            'description':
                'edit 必填: 每项只有 start_line(1-based)、end_line(可选)、content。范围包含首尾行，按 start_line 从大到小应用，失败时整批不写盘。',
            'items': {
              'type': 'object',
              'properties': {
                'start_line': {
                  'type': 'integer',
                  'minimum': 1,
                  'description': '要替换的起始行号，1-based，必填。',
                },
                'end_line': {
                  'type': 'integer',
                  'minimum': 1,
                  'description': '要替换的结束行号，1-based，包含本行；省略时等于 start_line。',
                },
                'content': {
                  'type': 'string',
                  'description': '替换内容；空字符串删除范围。插入内容时保留原行文本。',
                },
              },
      ...
      'parameters': {
        ...
        'properties': {
          'action': {
            ...
            'description':
                '操作: pick(打开系统文件选择器,会弹出系统 UI)/release(释放 picker id)/read(读,可分页+grep)/read_attr(查属性)/edit(按 1-based 行号替换,改代码首选)/write(覆盖写)/append(追加)/delete(删,仅限工作目录)/rename(改名,仅限工作目录)/list_dir(列目录,仅限工作目录)',
          },
          'path': {
            'type': 'string',
            'description':
          ...
          'edits': {
            'type': 'array',
            'description':
                'edit 必填: 每项含 start_line(必填)、end_line(可选)、content，按行号从大到小批量替换。',
            'items': {
              'type': 'object',
              'properties': {
                'start_line': {
                ...
                'end_line': {
                  'type': 'integer',
                  'minimum': 1,
                  'description': '结束行号，包含本行；省略时只编辑 start_line。',
                },
                'content': {
                  'type': 'string',
                  'description': '替换内容；空字符串删除指定行。插入内容时保留原行文本。',
                },
              },
              'required': ['start_line', 'content'],
            },
  ...
  List<EditOp> _parseEdits(dynamic rawEdits) {
    if (rawEdits is! List || rawEdits.isEmpty) {
      throw ToolException('"edits" is required and must be a non-empty array');
    }
    final ops = <EditOp>[];
    for (var i = 0; i < rawEdits.length; i++) {
      ...
      if (entry is! Map) {
        throw ToolException(
          'edits[$i] must be an object with start_line/end_line/content',
        );
      }
      try {
  ...
  Future<String> _executeMobile(
    ...
    switch (action) {
      ...
      case 'read':
        ...
        try {
          final text = TextFileData.decode(bytes).text;
          return jsonEncode(
      ...
      case 'edit':
        ...
        return _editResultOrThrow(path: path, result: result);
  ...
  Future<String> _editDesktop(String path, List<EditOp> ops) async {
    ...
    try {
      ...
      if (!application.result.ok) {
        return _editResultOrThrow(path: path, result: application.result);
      }
      ...
      final refreshedBytes = await file.readAsBytes();
      if (!_bytesEqual(updatedBytes, refreshedBytes)) {
        return _editResultOrThrow(
          path: path,
          result: EditResult.error(
            code: 'WRITE_VERIFY_FAILED',
            message:
                'file content did not match the requested edit after write',
            sizeBefore: bytes.length,
  ...
  ///     numbered so the model can pass the returned 1-based line
  ///     numbers directly to an `edit` call.
  ...
  Map<String, dynamic> _buildReadEnvelopeText({
    ...
    final allLines = splitTextLines(text);
  ...
  String _editResultOrThrow({
    required String path,
    required EditResult result,
  }) {
    final payload = jsonEncode(_editResultToJson(path: path, result: result));
    if (!result.ok) throw ToolException(payload);
    return payload;
  }
