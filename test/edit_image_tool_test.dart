...
void main() {
  ...
  group('ImageEditService', () {
    ...
    test(
      'rounded_corners: works on JPEG and produces a valid output',
      () async {
        final svc = ImageEditService(tempDir: tempDir);
        final result = await svc.edit(
          sourcePath: sourceJpeg.path,
        ...
        // JPEG preserves no alpha, but `_roundedCorners` still
        // runs the mask: the JPEG encoder composites the
        // transparent corners onto its default white background,
        // so the file must still be a readable, well-sized JPEG.
        expect(result.format, 'jpeg');
