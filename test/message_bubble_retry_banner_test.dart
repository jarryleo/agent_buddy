...
void main() {
  group('MessageBubble auto-retry banner', () {
    testWidgets('renders the retry banner with the live countdown when '
        'isRetrying is true', (tester) async {
      ...
      // drift.
      final expectedSeconds = (nextAt.difference(DateTime.now()).inSeconds)
          .clamp(0, 7);
      ...
      expect(
        ...
        findsOneWidget,
        reason:
            'expected banner text "$expected" with attempt=2 and '
            'countdown around ${expectedSeconds}s',
      );
    });

    testWidgets('renders NO retry banner when retryAttempt is 0', (
      tester,
    ) async {
    ...
    testWidgets('shows a fresh countdown label after nextAttemptAt updates', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          ...
          home: Scaffold(
            body: Builder(
              builder: (context) {
                return MessageBubble(message: m1, onCopy: (_) {});
              },
            ),
          ),
        ),
      ...
      final liveSeconds = rebuilt.nextRetryAt!
      ...
      expect(
        find.byWidgetPredicate(
          (w) =>
              w is Text &&
              w.data ==
                  l10n.chatRetryStatus('1', liveSeconds.clamp(0, 2).toString()),
        ),
        findsOneWidget,
    ...
    testWidgets('banner is hidden once the bubble transitions back to '
        'streaming state (next attempt started)', (tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          ...
          locale: const Locale('zh'),
          home: Scaffold(
            body: MessageBubble(message: midRetry, onCopy: (_) {}),
          ),
        ),
      );
      await tester.pump();
      ...
      await tester.pumpWidget(
        MaterialApp(
          ...
          locale: const Locale('zh'),
          home: Scaffold(
            body: MessageBubble(message: streaming, onCopy: (_) {}),
          ),
        ),
      );
      await tester.pump();
