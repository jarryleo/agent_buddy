...
void main() {
  group('RemindersTool defaults', () {
    ...
    test(
      'isEnabledByDefault is false so the picker is the user\'s '
    ...
    test(
      'isEnabledByDefault is false on every platform where the tool '
  ...
  group('ToolBase.isEnabledByDefault default', () {
    test(
      'defaults to true so the existing tools (fetch_web, current_time, '
      ...
      () {
        ...
        for (final tool in ToolRegistry.all) {
          if (tool.id == 'reminders') {
            expect(tool.isEnabledByDefault, isFalse,
